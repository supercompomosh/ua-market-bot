...
def get_admin_kb(user_id, has_photo=False):
    ...
    kb = [
        [types.InlineKeyboardButton(text="✅ Опублікувати", callback_data=f"post_{suffix}_{user_id}")],
...
async def city_selected(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"📍 {message.text}. Надішліть фото товару з описом (або просто текст):")
...
async def process_ad(message: types.Message, state: FSMContext):
    ...
    if message.photo:
        ...
        await bot.send_photo(ADMIN_ID, photo_id, caption=f"📥 ЗАЯВКА (ФОТО):\n\n{full_text}", 
                             reply_markup=get_admin_kb(message.from_user.id, True), parse_mode="Markdown")
    else:
        await bot.send_message(ADMIN_ID, f"📥 ЗАЯВКА:\n\n{full_text}", 
                               reply_markup=get_admin_kb(message.from_user.id, False), parse_mode="Markdown")
...
async def approve(callback: types.Callback_query):
    ...
    # Очищаем текст от префикса модерации
    clean_text = callback.message.caption or callback.message.text
    clean_text = clean_text.replace("📥 ЗАЯВКА (ФОТО):\n\n", "").replace("📥 ЗАЯВКА:\n\n", "")
